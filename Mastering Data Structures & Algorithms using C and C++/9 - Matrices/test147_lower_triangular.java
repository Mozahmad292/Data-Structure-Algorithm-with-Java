//lower triangular : square matrix : n by n
//1st method : using row-major indexing
//in mathematics indes starts from 1 onwards  : this got handled/coped up

class test147_lower_triangular{
  
  private int n; // matrix dimentions n*n//private emni
  private int[] A;  //1d array to store the lower triangular non 0 values
  int index;
  test147_lower_triangular(int tmp){ //constructor 
    this.n=tmp;  //this ="to the current object"
    A=new int[(n*(n+1))/2];
  }
  
  //
  void set(int i, int j, int x){
    if(i>=j){
      index=((i*(i-1))/2)+(j-1);
      A[index]=x;
     }
  }
  
  
  //get value at [i][j]
  int get(int i, int j){
    if(i>=j){ //just making sure if its a lower triangle positions
      index=((i*(i-1))/2)+(j-1);
      return A[index];
    }else{
      return 0; //others are 0s
    }
  }
  
  
  //print current status of matrix using 1d array off course  
  void display(){
    for(int i=1;i<=n;i++){
      
      for(int j=1;j<=n;j++){
        
        if(i>=j){
          index=((i*(i-1))/2)+(j-1);
          System.out.print(A[index]+" ");
        }else{
          System.out.print("0 ");
        }
        
      }
      
      System.out.println();
    }
  } 
  
//    void tmp(){
//    for(int i=0;i<A.length;i++){
//    System.out.print(A[i]+" ");
//    }
//  }
  
}